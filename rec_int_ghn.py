...
services_df = pd.read_csv('service_list2.csv')
...
users_with_enough_interactions_df = users_interactions_count_df[users_interactions_count_df >= 4].reset_index()[['User_Id']]
...
tfidf_matrix = vectorizer.fit_transform(services_df['Service_Name'] + " " + services_df['Topic'] )
...
print pd.DataFrame(sorted(zip(tfidf_feature_names, 
                        user_profiles[3333].flatten().tolist()), key=lambda x: -x[1])[:20],
...
#print item_popularity_df.head(20)
print hybrid_recommender_model.recommend_items(60236, topn=20, verbose=True)


def recommend( User_Id):
    User_Id=int(User_Id)
    found=0
    for index, row in users_with_enough_interactions_df.iterrows():
        #print row['User_Id']
    ...
    if (found):
        ...
        rec=item_popularity_df.head(20)
        rec=rec['Service_Id']
        rec=rec.tolist()
        rec=json.dumps(rec)
        return rec

print recommend(127004)
